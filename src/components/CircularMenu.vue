<template>
    ...
    <div v-for="(n,key) in menuCount" class="sub-menu" :key="key" :style="getSubMenu(n-1)">
        <router-link v-if="menuUrlList[n-1].isLink" :to="menuUrlList[n-1].url" :target="openNewTab" :style="subMenuStyle" @mouseover.stop="mouseEnterSubMenu" @mouseout.stop="mouseOutSubMenu" @click="toggleMenu">
...
<script>
  ...
  computed:{
    ...
    quickMenuStyle(){
      const topPosition = {top:'5px'}, 
      bottomPosition={bottom:'5px'},
      leftPosition = {left:'5px'},
      rightPosition = {right:'5px'}
  ...
  methods:{
    getSubMenu(n){
      ...
      return {top:menuPosition[n][0]*0.4+'px',left:menuPosition[n][1]*0.4+'px'}
    },
    toggleMenu(){
